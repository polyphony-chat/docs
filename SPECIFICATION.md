...
Challenge strings are generated by servers. They are a random, unique string of at least 64, and not more than 256 alphanumeric characters, which is used to verify that the user is in possession of their private identity key. Challenge strings have a lifetime in seconds, after which the challenge can no longer be completed. The lifetime always has to be indicated. The string is signed by the user's private identity key, and the signature is sent to the server, along with the user's ID-Cert. By verifying the signature of the challenge string with the user's ID-Cert, the server can verify that the user is in possession of their private identity key, and is who they claim to be.
